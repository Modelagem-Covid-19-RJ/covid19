...
def passo_matricial_kappa(num_pop, populacao, T_prob_nao_infeccao,
        ...
        for i, v in enumerate(pop_novos_infectados):
            if pop_novos_infectados[i] == 1:
                new_temp = np.concatenate((new_temp, [[0,i]]))
...
#         pop_novos_recuperados = np.select([pop_recuperando > prob_nao_recuperacao], [np.ones(num_pop)])
        ...
        return populacao_nova, new_temp[1:]

def evolucao_matricial_kappa(pop_0, tempo_infeccao_0, duracao_infeccao, G, tempos, num_sim, show=''):
    """Evolução temporal da epidemia em um grafo estruturado.
...
#           prob_nao_recuperacao = np.exp(-gamma*dt)
            ...
            populacao, tempo_infeccao = passo_matricial_kappa(num_pop, populacao,
                                                T_prob_nao_infeccao, 
                                                tempo_infeccao, duracao_infeccao)
            S = np.hstack([S, np.count_nonzero(populacao==1)])
            I = np.hstack([I, np.count_nonzero(populacao==2)])
